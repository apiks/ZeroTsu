...
//Adds a warning to a user in memberInfo.json without telling him
func AddWarningHandler(s *discordgo.Session, m *discordgo.MessageCreate) {
	...
	if ch.GuildID == config.ServerID {
		...
		if strings.HasPrefix(m.Content, config.BotPrefix) {
			...
			//Checks if BotPrefix + addwarning was used
			if strings.HasPrefix(messageLowercase, config.BotPrefix+"addwarning ") && (messageLowercase != (config.BotPrefix + "addwarning")) {
				...
				if err != nil && user != "" {
					...
					//Adds warnings to user memberInfo
